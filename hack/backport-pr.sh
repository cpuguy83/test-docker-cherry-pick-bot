...
backport() {
    (
        ...
        git fetch "${remote}" $branch:$branch || return 2
        git worktree add -b "${new_branch}" "${dir}" $branch || return 3

        cd "${dir}"
        for i in ${commits}; do
            git cherry-pick -x -s $i || return 3
        done

        git push -f "${remote}" "${new_branch}" || return 4
